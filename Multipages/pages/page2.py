...
layout = html.Div([
    # Store user-added tickers in the browser's memory# page2
...
def update_chart(submit_clicks, clear_clicks, new_ticker, stored_tickers):
    triggered_id = dash.ctx.triggered_id
    message = ""
    correlation_component = [] # Initialize as empty
    ...
    # Use the non-normalized closing prices for correlation
    close_prices = yf.download(
        all_tickers, start="2024-01-01", end="2025-01-01", auto_adjust=True, progress=False
    )['Close']
    ...
    close_prices.dropna(axis=1, how='all', inplace=True)
    if close_prices.empty:
        if not stored_tickers and triggered_id != 'clear-button':
             message = f"Could not find data for ticker: {new_ticker}"
        return px.line(title="Asset Performance (Normalized)"), stored_tickers, message, []
